...
- We must advertise a new signing key in our JWK Set for at least `T_cache` seconds before we start
  ...
  the new key, resulting in spurious verification failures.
- We must continue to advertise an old signing key in our JWK Set for at least `T_tokens` seconds
  before removing it entirely. If we don't, RPs who refresh their cache of our JWK Set may treat tokens
  signed with that key as invalid before they expire, resulting in supurious verification failures.
